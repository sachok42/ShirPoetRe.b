...
style_fit.py
────────────
Score candidate words against a text's stylistic fingerprint.

Public API
----------
rank_words_by_style(text, candidates) -> list[tuple[str, float]]
"""

from dataclasses import dataclass

from poem_utils import _clean_words, _syllable_count, _word_sentiment
...
class StyleProfile:
    ...
    formality_ratio: float      # share of words longer than 6 characters


def build_style_profile(text: str) -> StyleProfile:
    """Summarise the stylistic fingerprint of *text*."""
...
def style_score(word: str, profile: StyleProfile) -> float:
    """
    Return a 0–1 fitness score for how well *word* matches *profile*.
    Higher = better fit.
    """
